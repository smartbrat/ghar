...
| 14 | **Mobile search-pill size unified** | Compact pill on `#mobSearchTrigger` + `#mobSearchRow` now applies to every page on phones — was previously gated behind `body.simple-nav` so only inner pages got it. The homepage used to render a taller pill. Final tuned values: `padding: 18px 28px; line-height: 1.2` on the trigger, `6px 20px 8px` on the row. Single source in `styles.css`; no `body.simple-nav` qualifier left. |
| 15 | **Search-trigger family unified under `.search-trigger`** | The four trigger surfaces (`#mobSearchTrigger`, `#midSearchTrigger`, `#midSmallTrigger`, `#compactSearch`) used to be styled by a 4-ID grouped rule inlined in `index.html`'s `<style>` block. Each now carries `class="search-trigger"`; one canonical rule on `.search-trigger` / `.search-trigger:hover` in `styles.css`. Icon-stroke + label-color sub-rules became descendant selectors on the same class. Add the class to any new search-entry surface — don't stamp a new ID-keyed rule. |

Commits: `b1e7d2a` (URL contract) · `4b64494` (modal chassis reuse + desktop card) · `4ca4a7b` (clickSlopPx + card consolidation) · `27382f3` (Recent Searches) · `1a9d730` (Sign In full-screen) · `109d1ab` (pill size unified, first pass) · this push (`.search-trigger` canonical class + final padding/line-height tune).
