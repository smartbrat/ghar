...
function mobRenderAcSuggestions(query){
  ...
  if(locs.length){h+=`<div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#9ca3af;margin:10px 4px 4px">${query.trim()?"Locations":"Popular Localities"}</div>`;locs.forEach(loc=>{h+=mobLocRowHTML(loc);});}
  if(projs.length){h+=`<div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#9ca3af;margin:10px 4px 4px">${query.trim()?"Projects":"Popular Projects"}</div>`;projs.forEach(proj=>{h+=`<div class="mob-ac-item" style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:12px;cursor:pointer;font-size:14px" onclick="mobSelectProject('${proj.id}')"><span>🏢</span><div><div style="font-weight:500">${escapeHtml(proj.name)}</div><div style="font-size:12px;color:#6b7280">${escapeHtml(proj.micro)}</div></div></div>`;});}
  if(pins.length){h+=`<div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#9ca3af;margin:10px 4px 4px">${query.trim()?"Pincodes":"Popular Pincodes"}</div>`;pins.forEach(pin=>{h+=`<div class="mob-ac-item" style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:12px;cursor:pointer;font-size:14px" onclick="mobSelectPin('${pin.id}')"><span>📮</span><div><div style="font-weight:500">${escapeHtml(pin.code)}</div><div style="font-size:12px;color:#6b7280">${escapeHtml(pin.area)}</div></div></div>`;});}
